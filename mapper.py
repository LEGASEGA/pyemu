class Mapper:
    ...
    def __init__(self, prg_banks: int, chr_banks: int):
        ...
        self.mirroring = 0
        self.chr_dirty = True  # NEW: Flag to tell PPU to rebuild CHR LUT
...
class Mapper1(Mapper):
    ...
    def _update_mirroring(self):
        ...
        if mirroring_mode == 2: self.mirroring = 1   # Vertical
        elif mirroring_mode == 3: self.mirroring = 0 # Horizontal
        elif mirroring_mode == 0: self.mirroring = 2 # Single Screen 0
        elif mirroring_mode == 1: self.mirroring = 3 # Single Screen 1

    def ppu_map_read(self, addr: int) -> tuple[bool, int]:
        if 0x0000 <= addr <= 0x1FFF:
            if self.chr_banks == 0: # CHR RAM
                if self.control & 0x10: # 4KB mode
                    bank_0 = self.chr_bank_0 & 0x01
                    bank_1 = self.chr_bank_1 & 0x01
                    if addr <= 0x0FFF:
                        return True, (bank_0 * 0x1000) + (addr & 0x0FFF)
                    else:
                        return True, (bank_1 * 0x1000) + (addr & 0x0FFF)
                else:
                    return True, addr & 0x1FFF
            else: # CHR ROM
                chr_mask = (self.chr_banks * 2) - 1
                if self.control & 0x10: # 4KB mode
                    if addr <= 0x0FFF:
                        return True, ((self.chr_bank_0 & chr_mask) * 0x1000) + (addr & 0x0FFF)
                    else:
                        return True, ((self.chr_bank_1 & chr_mask) * 0x1000) + (addr & 0x0FFF)
                else: # 8KB mode
                    bank = (self.chr_bank_0 & chr_mask) & 0xFE
                    return True, (bank * 0x1000) + (addr & 0x1FFF)
        return False, 0
...
class Mapper2(Mapper):
    ...
    def cpu_map_read(self, addr: int) -> tuple[bool, int]:
        if 0x8000 <= addr <= 0xBFFF:
            return True, (self.selected_prg * 0x4000) + (addr & 0x3FFF)
...
class Mapper4(Mapper):
    """MMC3 Mapper for Super Mario Bros. 3"""
    def __init__(self, prg_banks: int, chr_banks: int):
        super().__init__(prg_banks, chr_banks)
        self.target_reg = 0
        self.prg_mode = 0
    ...
    def cpu_map_read(self, addr: int) -> tuple[bool, int]:
        ...
        if 0x8000 <= addr <= 0xFFFF:
            ...
            mapped_addr = (self.prg_banks_map[bank_idx] * 0x2000) + (addr & 0x1FFF)
    ...
    def ppu_map_read(self, addr: int) -> tuple[bool, int]:
        if 0x0000 <= addr <= 0x1FFF:
            ...
            offset = addr & 0x03FF
            return True, (self.chr_banks_map[bank_idx] * 0x0400) + offset
        return False, 0
